...
def wiebe(CAD, CAD_step, Spark, combustion_duration, m):
    ...
    CAD_spark = 360 - Spark  # CAD at ignition

    xb_i = 1 - np.e**(-a*((CAD - CAD_spark) / combustion_duration)**(m + 1))
    xb_i_1 = 1 - np.e**(-a*((CAD + CAD_step - CAD_spark) / combustion_duration)**(m + 1))
...
def crank_slider(CAD, SHR, Tatm, engine_parameters = engine_parameters):
    ...
    #Heat Transfer Area
    #FIX: bore and piston_position are in mm, so this expression is in mm^2. hcg (Woschni
    #heat-transfer coefficient) is in W/(m^2*K), so Ah must be converted to m^2 before it is
    #ever multiplied by hcg, or Qloss comes out ~1e6x too large.
    Ah_mm2 = (np.pi*bore**2)/2 + np.pi*bore*piston_position
    Ah = Ah_mm2 / 1e6  # m^2
...
def combustion_analysis(CAD, CAD_step, P_i, T_i, Spark, combustion_duration, lmnbda, rpm, T_wall, fuel_type, m, location, engine_parameters = engine_parameters, fuel_properties = fuel_properties, weather_data= weather_data):
    ...
    _, _, v_i, v_i_1, Ah = crank_slider(CAD, k, weather_data[location]['t_atm'])
...
def douaund_eyzat(ON, p_cad, t_cad):
    ...
    p_cad_bar = p_cad / 1e5  # Pa -> bar

    IDT = c1 * (ON/100)**c2 * p_cad_bar**(-c3) * np.exp(c4/t_cad)
...
def hoepke(x_egr, p_cad, t_cad):
    ...
    IDT = c1 * (p_cad_bar/t_cad)**-c2 * (1 - x_egr)**-c3 * np.e**(c4/t_cad)
...
def chen_zheng(lmnbda, x_egr, p_cad, t_cad):
    ...
    IDT = c1 * (p_cad_bar/t_cad)**-c2 * (1 - x_egr)**-c3 * lmnbda**-c4 * np.e**(c5/t_cad)
...
#FIX: ETHANOL_FRACTION/OCTANE_FRACTION removed as hardcoded globals; they are now
#pulled from fuel_properties[fuel_type] so the charge composition actually matches
#whichever fuel is selected (previously every fuel was treated as E85 regardless
#of fuel_type, e.g. '93' pump gas was still built as an 85% ethanol blend).
...
for spark in tqdm(range(0,20), desc= 'varying spark timing...'):
    ...
    for CAD in np.arange((engine_parameters['combustion_characteristics']['CADivc']+ 180), (360 - spark + combustion_duration_i),1):
        ...
        if CAD < 360-spark: 
            ...
            shr = Cp_u / (Cp_u - R)
            p_cad, t_cad, v_cad, v_cad_1, Ah = crank_slider(CAD, shr, weather_data[location]['t_atm'])

        else:
            #FIX: this call's argument order did not match combustion_analysis()'s signature
            #(CAD, CAD_step, P_i, T_i, Spark, combustion_duration, lmnbda, rpm, T_wall,
            #fuel_type, m, location). The old call put Tatm where T_wall belongs, T_wall_i
            #where fuel_type belongs, fuel_type where m belongs, and m_i where location
            #belongs - with location itself missing. That misalignment either crashes
            #(e.g. fuel_properties[T_wall_i] -> KeyError) or silently runs the physics with
            #swapped values. It also passed the whole combustion_duration Monte Carlo array
            #instead of this iteration's scalar combustion_duration_i. Both are fixed below.
        ...
        #Ignition Delay Timing
        #FIX: these previously passed the full 200-sample Monte Carlo `rpm` array instead
        #of the scalar `rpm_i` drawn for this spark-timing run, which fed an array into
        #livengood_wu()'s omega calculation and produced 200 values per call instead of one.
        livengood_wu_integral['douaund_eyzat'][spark].append((livengood_wu(rpm_i, douaund_eyzat(ON, p_cad, t_cad), CAD_step)) + sum(livengood_wu_integral['douaund_eyzat'][spark]))
        livengood_wu_integral['hoepke'][spark].append(livengood_wu(rpm_i, hoepke(0, p_cad, t_cad), CAD_step) + sum(livengood_wu_integral['hoepke'][spark]))
        livengood_wu_integral['chen_zheng'][spark].append(livengood_wu(rpm_i, chen_zheng(lmnbda,0,p_cad,t_cad), CAD_step) + sum(livengood_wu_integral['chen_zheng'][spark]))

print(pressure[12])
# print(temperature[12])
# print(livengood_wu_integral['chen_zheng'][12])

#FIX: the original plot passed the whole livengood_wu_integral['chen_zheng'] dict of lists
#(keyed by spark timing) instead of a single spark's list, and CADs[spark] (one entry per
#CAD step) was one element shorter than the result lists (which start with a seeded [0]
#before the loop appends once per step). Indexed by spark timing 12 and sliced to drop that
#seed element so x and y line up.
plt.plot(CADs[12], pressure[12][:-1])
plt.axvline(x = 360-12, color= 'red', linestyle= '--')
plt.xlabel('Crank Angle Degree')
plt.ylabel('Livengood-Wu Integral (Chen-Zheng)')
plt.show()
